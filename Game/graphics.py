...
class Text(GraphicsObject):
    ...
    def setFace(self, face):
        # Accept any font family name supported by the OS/Tkinter
        f,s,b = self.config['font']
        self._reconfig("font",(face,s,b))

    def setFont(self, font_tuple):
        """Set the whole font tuple: (family, size, style)
        Example: setFont(('Segoe UI', 20, 'bold'))
        """
        if (isinstance(font_tuple, (list, tuple)) and len(font_tuple) == 3):
            self._reconfig("font", tuple(font_tuple))
        else:
            raise GraphicsError(BAD_OPTION)
